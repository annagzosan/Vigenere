...
#as we see we numbers,we can clearly tell that the "big" number are 150<
#how often big numbers occur
last_seen=-1
possible_key_length=[]
for i in range(1,len(coincidences)):
    if coincidences[i]>150:     #150 cause we see that this is the smallest "big" number                                     ----CHANGE
        if (last_seen>0):
...
for m in range(7): # 7 times so we can find the 7 numbers of the key                                                     ----CHANGE
    max_=-1
    count=0;
